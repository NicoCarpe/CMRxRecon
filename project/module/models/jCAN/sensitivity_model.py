...
# Transpose blocks were not used in this code origionally, so make sure the change is necessary
...
class SMEB3D(nn.Module):
    ...
    def forward(
        ...
        ACS_mask = ACS_mask[None, None, None, None, :].to(masked_kspace)
        ...
        B, C, T, H, W = ACS_images.shape
        batched_channels = ACS_images.reshape(B * C, 1, T, H, W)
        
        #### gated ####
        sensitivity, gate = self.norm_net(batched_channels)
        gate = gate.reshape(B, C, T, H, W)
        
        sensitivity = sensitivity.reshape(B, C, T, H, W)
...
class Unet3D(nn.Module):
    ...
    def forward(self, image: torch.Tensor) -> torch.Tensor:
        ...
        output_ = utils.gaussian_smooth(output.view(-1, 1, *output.shape[-3:]), sigma=torch.clamp(self.stds[cnt], min=1e-9, max=7))
        output = output_.view(output.shape)
        output = F.interpolate(output, scale_factor=(1, 2**(self.num_pool_layers-3), 2**(self.num_pool_layers-3)))
