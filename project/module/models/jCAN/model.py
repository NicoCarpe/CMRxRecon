...
import fD2RT3D  # Make sure to update this import if the module name is different
...
def TV_loss(img, weight):
    bs_img, c_img, t_img, h_img, w_img = img.size()
    tv_h = torch.pow(img[:, :, :, 1:, :] - img[:, :, :, :-1, :], 2).sum()
    tv_w = torch.pow(img[:, :, :, :, 1:] - img[:, :, :, :, :-1], 2).sum()
    return weight * (tv_h + tv_w) / (bs_img * c_img * t_img * h_img * w_img)
...
class ReconModel(BaseModel):
    def __init__(self, *args, **kwargs):
        ...
        self.net_R = fD2RT3D.fD2RT3D(coils=self.cfg.coils, img_size=self.cfg.img_size, num_heads=self.cfg.num_heads, 
                                     window_size=self.cfg.window_size, depths=self.cfg.depths, patch_size=self.cfg.patch_size, 
                                     embed_dim=self.cfg.embed_dim, mlp_ratio=self.cfg.mlp_ratio, qkv_bias=True, qk_scale=None, 
                                     drop=0., attn_drop=0., drop_path=0., norm_layer=nn.LayerNorm, n_SC=self.cfg.n_SC, 
                                     num_recurrent=self.cfg.num_recurrent, sens_chans=self.cfg.sens_chans, 
                                     sens_steps=self.cfg.sens_steps, ds_ref=self.cfg.ds_ref).to(self.device)

    def set_input_noGT(self, Target_img_sampled):
        B, C, T, H, W = Target_img_sampled.shape
        self.Target_f_rss = torch.zeros([B, 1, T, H, W], dtype=torch.complex64)
        self.Target_Kspace_f = torch.zeros([B, C, T, H, W], dtype=torch.complex64)
    ...
    def set_input_GT(self, Target_img_f):
