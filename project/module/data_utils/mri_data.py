...
class CombinedCmrxReconDataset2024(torch.utils.data.Dataset):
    """
    ...
    def __init__(
        ...
        if not (
            ...
            raise ValueError(
                "Lengths of roots, transforms do not match"
            )
...
class CmrxReconDataset2024(torch.utils.data.Dataset):
    def __init__(
        self,
        ...
        self.recons_key = "reconstruction_rss"  # since we are only using Multicoil in this dataset
        self.raw_samples = []
        ...
        if dataset_cache.get(root) is None or not use_dataset_cache:
            ...
            for data_type in data_type_list:
                task_dir = os.path.join(root, data_type, 'TrainingSet/', 'h5_FullSample')
