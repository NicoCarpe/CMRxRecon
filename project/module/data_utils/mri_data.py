...
class CmrxReconSliceDataset2024(torch.utils.data.Dataset):
    ...
    if dataset_cache.get(root) is None or not use_dataset_cache:
        ...
        # Process each contrast and view
        contrasts = ['Aorta', 'Cine', 'Mapping', 'Tagging']

        # Loop through each contrast
        for contrast in contrasts:
            contrast_folder = Path(root) / contrast
            if contrast_folder.exists():
                # Loop through each view within the contrast
                for view in views:
                    view_folder = contrast_folder / view
                    if view_folder.exists():
                        files += list(view_folder.iterdir())
        ...
        if dataset_cache.get(root) is None or not use_dataset_cache:
            ...
            for fname in sorted(files): 
                ...
                for slice_ind in range(num_slices):
                    raw_sample = CMRxReconRawDataSample2024(fname, slice_ind, metadata)
