...
import fastmri
from data.mri_data2D import CombinedSliceDataset, SliceDataset
...
class DataModule(pl.LightningDataModule):
    ...
    def _create_data_loader(
        ...
        sample_rate: Optional[float] = None,
        volume_sample_rate: Optional[float] = None,
        split_train_val: bool = False  # Flag to control the splitting of the train data into train / val
        ...
        # Dynamic train/val splitting logic
        if is_train and split_train_val:
            dataset = CombinedSliceDataset(
                roots=[self.data_path / f"{self.challenge}_train"],
                transforms=[data_transform],
                challenges=[self.challenge],
                sample_rates=[sample_rate],
                volume_sample_rates=[volume_sample_rate],
                use_dataset_cache=self.use_dataset_cache_file,
                raw_sample_filter=raw_sample_filter,
            )

            # Split the dataset into train/val dynamically
            train_size = int((1 - self.val_split_ratio) * len(dataset))
            val_size = len(dataset) - train_size
            seed = torch.Generator().manual_seed(42)
            train_dataset, val_dataset = torch.utils.data.random_split(
                dataset, [train_size, val_size], generator=seed
            )

            return train_dataset, val_dataset

        # If not dynamically splitting or handling val/test
    ...
    def train_dataloader(self):
        train_dataset, _ = self._create_data_loader(self.train_transform, data_partition="train", split_train_val=True)
    ...
    def val_dataloader(self):
        _, val_dataset = self._create_data_loader(self.val_transform, data_partition="train", split_train_val=True)
        return torch.utils.data.DataLoader(
            val_dataset,
            batch_size=self.batch_size,
            num_workers=self.num_workers,
