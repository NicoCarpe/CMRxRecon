...
class CmrxReconSliceDataset2024(torch.utils.data.Dataset):
                        ...
                        for ks_file_info in os.scandir(full_ks_dir_path):
                            if ks_file_info.is_file() and ks_file_info.name not in ['.', '..', '.DS_Store']:
                                ...
                                with h5py.File(full_ks_file_path, 'r') as hf:
                                    ...
                                    num_slices, t, _, x, y = hf["kspace"].shape

                                    #TODO: checking for each file is inefficient, fix to check for each type of image (assuming consistent dimensions)
                                    if t > self.max_img_size[0]:
                                        self.max_img_size[0] = t
                                    if x > self.max_img_size[1]:
                                        self.max_img_size[1] = x
                                    if y > self.max_img_size[2]:
                                        self.max_img_size[2] = y
                                    ...
                                    for slice_ind in range(num_slices):
                                        ...
                                        for mask_path in os.scandir(hf['masks']):
                                            self.raw_samples.append(CMRxReconRawDataSample2024(full_ks_file_path, slice_ind, data_type, mask_path))
    ...
    def __getitem__(self, i: int):
        ...
        with h5py.File(str(fname),'r') as hf:
            kspace = np.squeeze(np.asarray(hf["kspace"][slice_ind]), axis=0)   # squeeze the slice dimension
            target = np.squeeze(np.asarray(hf[self.recons_key][slice_ind]), axis=0) if self.recons_key in hf else None
            attrs = dict(hf.attrs)
        
        # Determine the undersampling pattern and corresponding ACS region
        if 'Uniform' in mask_path:
            undersampling_pattern = 'Uniform'
            num_low_frequencies = [16] 
        elif 'Gaussian' in mask_path:
            undersampling_pattern = 'Gaussian'
            num_low_frequencies = [16] 
        elif 'Radial' in mask_path:
            undersampling_pattern = 'Radial'
            num_low_frequencies = [16, 16] 
        else:
            undersampling_pattern = None

        # Add the undersampling pattern, number of low frequencies, and slice indicies to the attrs
        attrs['undersampling_pattern'] = undersampling_pattern
        attrs['num_low_frequencies'] = num_low_frequencies
        attrs['slice_ind'] = slice_ind
        ...
        if self.transform is None:
            sample = (kspace, mask, target, attrs, str(fname), self.max_img_size)
        else:
            sample = self.transform(kspace, mask, target, attrs, str(fname), self.max_img_size)
